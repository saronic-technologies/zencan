...
/// Information about a node on the bus
...
impl NodeInfo {
    /// Create a new NodeInfo with just a node ID
...
// Removed a bunch of methods that were cluttering up this class; this class 
// now serves to run an async task that monitors the heartbeat message on the bus,
// which we can use to see if a device has timed out
