...
pub use zencan_common::object_model::DefaultValue;
use zencan_common::object_model::{AccessType, DataType, ObjectCode, PdoMappable, PdoMapping};

use snafu::ResultExt as _;
use snafu::Snafu;

fn deserialize_pdo_map<'de, D, T>(deserializer: D) -> Result<HashMap<usize, T>, D::Error>
where
    D: Deserializer<'de>,
    T: Deserialize<'de>,
{
...
pub struct SubDefinition {
    ...
    #[serde(default, deserialize_with = "deserialize_access_type")]
...
impl DeviceConfig {
    ...
    pub fn load_from_str(config_str: &str) -> Result<Self, LoadError> {
        let config: DeviceConfig = toml::from_str(config_str).context(TomlParsingSnafu)?;
...
fn deserialize_access_type<'de, D>(deserializer: D) -> Result<AccessType, D::Error>
where
    ...
    D: serde::Deserializer<'de>,
{
    ...
    if s == "boolean" {
        ...
        let size: usize = caps[1]
            .parse()
            .map_err(|_| D::Error::custom(format!("Invalid size for OctetString: {}", &caps[1])))?;
